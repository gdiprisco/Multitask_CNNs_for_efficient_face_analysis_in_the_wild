...
class DataGenerator(tensorflow.keras.utils.Sequence): # TODO VIGILANTE
    ...
    def __init__(self, data, target_shape, with_augmentation=True, batch_size=64, custom_augmentation=None, num_classes=None, preprocessing='full_normalization', fullinfo=False, rebalance=False, rebalance_factor=1):#, explode_label=False):
        ...
        if rebalance:
            self.total_data, self.data = data, None
            self._check_total_data()
            self.rebalance_factor = rebalance_factor
            self.rebalance_end_index = self._get_relative_magnitude()
            self.rebalance_index = 0
        else:
            self.data, self.total_data = data, None
    ...
    def on_epoch_end(self):
        ...
        self.cur_index = 0
        if self.rebalance:
            print("\nRebalancing data part {} of {}...".format(self.rebalance_index+1, self.rebalance_end_index))
            self.data = self._get_rebalance_data()
            print("Rebalanced data. {} samples.".format(len(self.data)))
        print('Shuffle set')
        np.random.shuffle(self.data)
        self.mutex.release()

    def _check_total_data(self):
        if type(self.total_data) is not list:
            raise Exception("Datagenerator: rebalance parameter asserted, passed data must be in the list form.")
        elif len(self.total_data) > 2:
            raise Exception("Datagenerator: rebalance parameter asserted, only two datasets enseble is currently supported")
        elif len(self.total_data[0]) < len(self.total_data[1]):
            print("Swapping datasets...")
            self.total_data[1], self.total_data[0] = self.total_data[0], self.total_data[1]

    def _get_relative_magnitude(self):
        return ceil(len(self.total_data[0])/len(self.total_data[1])/self.rebalance_factor)

   
    def _get_rebalance_data(self):
        if self.rebalance_index == 0:
            print("Shuffling major dataset...")
            np.random.shuffle(self.total_data[0])
            # np.random.shuffle(self.total_data[1])
        begin = len(self.total_data[1])*self.rebalance_factor*self.rebalance_index 
        self.rebalance_index += 1
        end = len(self.total_data[1])*self.rebalance_factor*self.rebalance_index
        print("Major dataset begin index {}, end index {}".format(begin, end))
        big_set = self.total_data[0][begin:end]
        if end >= len(self.total_data[0]):
            print("End index exceeds data size, circular remainder applied.")
            remainder = end-len(self.total_data[0])
            big_set += self.total_data[0][0:remainder+1]
            self.rebalance_index = 0
        print("Using {} elements in minor dataset.".format(len(self.total_data[1])))
        return big_set + self.total_data[1]
        

    def _categorical_mask(self, label, num_classes):
        if label == MASK_VALUE:
            return np.array([MASK_VALUE]*num_classes, dtype=np.float32)
        return np.array(keras.utils.to_categorical(label, num_classes=num_classes))


    def _masked_label(self, label):
        if isinstance(label,int) and self.num_classes>1:
            ...
            for i in range(0, len(label)):
                categorical = label[i] if self.num_classes[i] == 1 else self._categorical_mask(label[i], self.num_classes[i])
                extended_categorical.append(categorical)
            label = np.array(extended_categorical)
        return label
    ...
    def _load_item(self, d):
        ...
        if self.num_classes is not None:
            label = self._masked_label(label)
        frame = d['img']
        ...
        # elif self.explode_label:
        #     return ([img] + [l for l in label])
        return (img, label)
    ...
    def _load_batch(self, start_index, load_pairs=False):
        ...
        for index in range(start_index, start_index+size_of_this_batch):
            ...
            for j in range(len(item[1])):
                stuff[1][j].append(item[1][j]) #single label
